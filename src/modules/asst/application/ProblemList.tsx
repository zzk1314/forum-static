...
import {loadApplicationProblems} from "../async"
    ...
    loadApplicationProblems().then(res =>{
      if (res.code === 200) {
        ...
        this.context.router.push({
          pathname:"/login",
          query:{
            callbackUrl:`/asst/application/comment`
          }
        })
    ...
    return (
      <div className="problemContent">
        <div className="leftList" style={{position:'static'}}>
          <ProblemView chooseProblem={(problemId)=>this.chooseProblem(problemId)} problemList={problemList}/>
        </div>
