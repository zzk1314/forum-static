...
export default class ShowChallenge extends React.Component<any, any> {
  ...
  clickSubmitComment() {
    ...
    if(!comment) {
      ...
      dispatch(startLoad())
      submitComment(CommentType.Subject, submitId, comment).then(res => {
        dispatch(endLoad())
        if(res.code == 200) {
          let newArr = [];
          newArr.push(res.msg);
  ...
  render() {
    ...
    const { title, upName, upTime, role, signature, headImg, content, isMine, hasMore, requestCommentCount, request } = data
